...
const REPO_OWNER = 'criss-vevo';
const REPO_NAME = 'CRISS-MD';
const PLUGINS_FOLDER = '../criss/plugins'; // Folder where plugins are stored
...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        let pluginList = "📂 *CRISS MD Plugins:*\n\n";
...
async (conn, mek, m, { from, reply, args, senderNumber }) => {
    try {
        ...
        if (m.quoted && m.quoted.key.fromMe) {
            ...
            if (/📂 \*CRISS MD Plugins:\*/i.test(quotedText)) {
        ...
        const statusMessage = {
            image: { url: `https://files.catbox.moe/xro4ka.jpeg` }, // Replace with your image URL
