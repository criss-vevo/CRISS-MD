...
cmd({ 
  ...
  try { 
    ...
    if (!fs.existsSync('./.git')) { 
      ...
      execSync('git init'); 
      execSync('git remote add origin https://www.github.com/criss-vevo/CRISS-MD.git'); 
    } else { 
    ...
    if (localCommit === originCommit) { 
      await conn.sendMessage(from, { text: '*✅ Criss Md Bot is already up to date!*' }, { quoted: mek });
    } else { 
      ...
      await conn.sendMessage(from, { text: '*✅ Criss Md Bot updated successfully!*' }, { quoted: mek });
    }
  } catch (error) { 
    console.error(error); 
