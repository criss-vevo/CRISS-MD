...
cmd({
    ...
    if (args[0] === "on") {
        ...
        isFactEnabled = true;
        reply("✅ The daily fact feature is now enabled. I will send a fact every day at 6 AM (Zimbabwean time).");

        // Set the daily fact interval at 6 AM (Zimbabwean time)
...
cmd({
    ...
    try {
        if (args.length === 0) {
            return reply("❌ Please provide a timezone. Example: `.timezone Africs/Zimbabwe`");
        }
